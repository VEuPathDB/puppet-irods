...
values set for this module's `irods::provider::db_vendor`,
`irods::provider::db_name`, `irods::provider::db_user` and
`irods::provider::db_password` parameters.
...
(`irods::globals::provider_admin_user`). You should set this because the
...
(the one defined for `irods::globals::provider_admin_user`).
